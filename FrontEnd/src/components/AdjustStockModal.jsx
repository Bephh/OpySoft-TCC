        ...
        >
            ...
            >
                ...
                <div className="flex justify-between items-center border-b border-gray-700/50 pb-4 mb-6 flex-shrink-0">
                    <h3 className="text-2xl font-extrabold text-blue-400">Editar Componente</h3>
                    <button 
                        onClick={onClose} 
                    ...
                    <form id="adjust-form" onSubmit={handleSubmit} className="space-y-5">
                        ...
                        <div className="flex gap-5">
                            <div className="flex-1">
                                ...
                                {/* O campo de componente pode ser editável */}
                                <input type="text" name="component" id="component" value={formData.component} onChange={handleChange} required className={inputStyle} />
                            </div>
                            <div className="flex-1">
                                <label htmlFor="sku" className="block text-sm font-semibold text-gray-300 mb-1">SKU (Código)</label>
                                {/* SKU geralmente é apenas leitura para evitar inconsistências */}
                                <input type="text" name="sku" id="sku" value={formData.sku} readOnly className={`${inputStyle} cursor-not-allowed bg-gray-700/30`} />
                            </div>
                        </div>

                        {/* CATEGORIA (apenas leitura, ou permitindo alteração se for um requisito) */}
                        <div>
                            <label htmlFor="category" className="block text-sm font-semibold text-gray-300 mb-1">Categoria do Componente</label>
                            {/* Categoria geralmente é apenas leitura para evitar inconsistências */}
                            <input type="text" name="category" id="category" value={formData.category} readOnly className={`${inputStyle} cursor-not-allowed bg-gray-700/30`} />
                        </div>
                        ...
                        <div className="grid grid-cols-3 gap-5">
                            <div>
                                ...
                                <input type="number" name="price" id="price" value={formData.price} onChange={handleChange} required min="0" step="0.01" className={inputStyle} />
                            </div>
                            <div>
                                <label htmlFor="power" className="block text-sm font-semibold text-gray-300 mb-1">Consumo (Watts)</label>
                                <input type="number" name="power" id="power" value={formData.power} onChange={handleChange} required min="0" className={inputStyle} />
                            </div>
                            <div>
                                ...
                                <div className="grid grid-cols-3 gap-5">
                                    {showSocket && (
                                        <div>
                                            ...
                                            <input type="text" name="socket" id="socket" value={formData.socket} onChange={handleChange} className={inputStyle} />
                                        </div>
                                    )}
                                    {showRamType && (
                                        <div>
                                            <label htmlFor="ramType" className="block text-sm font-semibold text-gray-300 mb-1">Tipo de RAM</label>
                                            <input type="text" name="ramType" id="ramType" value={formData.ramType} onChange={handleChange} className={inputStyle} />
                                        </div>
                                    )}
                                    {showWatt && (
                                        <div>
                                            <label htmlFor="watt" className="block text-sm font-semibold text-gray-300 mb-1">Capacidade da Fonte (W)</label>
                                            <input type="number" name="watt" id="watt" value={formData.watt} onChange={handleChange} className={inputStyle} />
                                        </div>
                                    )}
                        ...
                        <div className="pt-2"> 
                            ...
                            <input type="text" name="supplier" id="supplier" value={formData.supplier} onChange={handleChange} required className={inputStyle} />
                        </div>
